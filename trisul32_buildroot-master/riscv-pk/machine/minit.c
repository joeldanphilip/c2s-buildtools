...
static void mstatus_init()
{
  ...
  if (supports_extension('V'))
    mstatus |= MSTATUS_VS;
        ...
        mstatus |= (MSTATUS_SIE); //MSTATUS_MIE |
  ...
  write_csr(mie, (MIP_MSIP | MIP_SEIP));
  //write_csr(mie, (MIP_MSIP | MIP_SEIP | MIP_MEIP));
...
void init_first_hart(uintptr_t hartid, uintptr_t dtb)
{
  ...
  //printm("\nMIP: 0x%08x", read_csr(mip));

  //printm("\nMIE: 0x%08x", read_csr(mie));

  //printm("\nMSTATUS: 0x%08x", read_csr(mstatus));
  //prci_test();
  memory_init();
  boot_loader(dtb);
}
