...
static void clint_done(const struct fdt_scan_node *node, void *extra)
{
  ...
  scan->done = 1;
  mtime = (void*)((uintptr_t)scan->reg + 0x580);
  ...
  for (int index = 0; end - value > 0; ++index) {
    ...
    if (hart < MAX_HARTS) {
      ...
      hls->ipi = (void*)((uintptr_t)scan->reg + 0x400 +index * 0x10);
      hls->timecmp = (void*)((uintptr_t)scan->reg + 0x490 + (index * 0x10));
...
{
  struct plic_scan *scan = (struct plic_scan *)extra;
  if (!strcmp(prop->name, "compatible") && fdt_string_list_index(prop, "cdac,plic-1.0.0") >= 0) {
    scan->compat = 1;
  } else if (!strcmp(prop->name, "reg")) {
    fdt_get_address(prop->node->parent, prop->value, &scan->reg);
  } else if (!strcmp(prop->name, "interrupts-extended")) {
...
#define ENABLE_BASE	0x018 //(for SIC) //0x08(for PLIC)
